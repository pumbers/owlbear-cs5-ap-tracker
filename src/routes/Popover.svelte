          ...
          <tr>
            <th>
              {#await playerRole then role}
                {#if role === "GM"}
                  <button class="w-4 m-1" title="Roll All" on:click={rollAll}
            ...
            <tr>
              ...
              <td class="text-center"
                ><div class="relative inline-block group w-full">
                  ...
                  <div class="hidden absolute w-15 z-10 group-hover:block bg-obr-violet">
                    {#each Array.from({ length: Math.min(item.ap, 10) }, (_, i) => i + 1) as spend}
                      <!-- svelte-ignore a11y-missing-attribute -->
                      <!-- svelte-ignore a11y-click-events-have-key-events -->
                      <a
                        class="block no-underline p-1 hover:bg-obr-violet-700 cursor-pointer"
